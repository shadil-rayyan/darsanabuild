...
import NorthAmericachapter from "../../../assets/chapter/saudichapter.png"; // Check this
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 mt-4">
        {chapters
          ...
          .map((chapter, index) => (
            <ChapterCard
              key={index}
              icon={<img src={chapter.image} alt={chapter.name} className="w-full h-full object-cover" />}
              title={chapter.name}
              description={`Explore more about ${chapter.name}`}
            />
          ))}
      </div>
    </div>
